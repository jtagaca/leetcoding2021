...
class Solution:
    ...
    def bfs(self, forest, sr, sc, tr, tc):
        ...
        queue = [(sr,sc,0)]
        seen = {(sr, sc)}
        #d is keeping track of the level that we are in and so even if we consider the same element twice 
        # we are doing a back tracking at this level since if that does not traverse 
        ...
        while queue:
            ...
            # we consider every neighbor and every possibility but only return those that are able to reach
            # the target
